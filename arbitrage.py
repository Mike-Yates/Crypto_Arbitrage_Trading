...
def performArbitrage(): 
    ...
    for dex_address in config['dex_addrs']: # loop through all the dexes. 
        ...
        after_holdings_1 = (qe+f*xd-f*kd/(yd+δt1)) * pe + (qt-δt1) * pt - g * pe
        after_holdings_2 = (qe+f*xd-f*kd/(yd+δt2)) * pe + (qt-δt2) * pt - g * pe
    ...
    if(winning_dex_index == -1):
        ...
        # output(ethAmt, tcAmt, fees, holdings)
        output(dex_values[winning_dex_index]['ether_amount'], dex_values[winning_dex_index]['token_amount'], dex_values[winning_dex_index]['f'], dex_values[winning_dex_index]['h_after'])
